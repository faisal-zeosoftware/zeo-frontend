...
@Component({
  selector: 'app-airticket-escalation',
  templateUrl: './airticket-escalation.component.html',
  styleUrl: './airticket-escalation.component.css'
})

export class AirticketEscalationComponent {
 ...
 this.dataSubscription = combineLatest([
  ...
  if (schema) {
                ...
                try {
                  ...
                  if (Array.isArray(permissionsData) && permissionsData.length > 0) {
                    ...
                    if (firstItem.is_superuser) {
                      ...
                      this.hasAddPermission = this.checkGroupPermission('add_airticket_escalation', groupPermissions);
                      console.log('Has add permission:', this.hasAddPermission);
        
                     this.hasDeletePermission = this.checkGroupPermission('delete_airticket_escalation', groupPermissions);
    ...
    resetSelectedGeneralReq() {
      ...
      if (confirm('Are you sure you want to reset the selected General Request(s)?')) {
        ...
        selectedIds.forEach(id => {
          this.employeeService.resetAirticketEsc(id).subscribe(
            () => {
    ...
    updateAssetType(): void {
      ...
      this.employeeService.updateAirticketEsc(this.editAsset.id, this.editAsset).subscribe(
        (response) => {
          alert('Airticket Escalation updated successfully!');
          this.closeEditModal();
    // combineLatest waits for both Schema and Branches to have a value
    this.dataSubscription = combineLatest([
      this.employeeService.selectedSchema$,
      ...
      console.log("Mapped employee_id:", this.editAsset.escalate_to);
              ...
              registerGeneralreq() {
                ...
                this.DepartmentServiceService
                    .updateEscalationLevelAir(selectedSchema, levelId, payload)
                    .subscribe(
                      (res) => {
    ...
    onRequestTypeChange(event: any) {
      ...
      this.DepartmentServiceService.getEscalationLevelsAirticket(selectedSchema, selectedTypeId)
        .subscribe(
          (result: any) => {
          ...
          (error) => {
            console.error('Error fetching escalation levels:', error);
              ...
              //   // Check if selectedSchema is available
              //   if (selectedSchema) {
              //     this.employeeService.getAllgeneralRequestEscalationsAirticket(selectedSchema).subscribe(
              //       (result: any) => {
              ...
              //   }
                ...
                fetchEmployees(schema: string, branchIds: number[]): void {
                  this.isLoading = true;
                  this.employeeService.getEscalationsAirticketNew(schema, branchIds).subscribe({
                    next: (data: any) => {
