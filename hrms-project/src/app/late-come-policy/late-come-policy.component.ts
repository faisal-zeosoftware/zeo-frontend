...
@Component({
  selector: 'app-late-come-policy',
  templateUrl: './late-come-policy.component.html',
  styleUrl: './late-come-policy.component.css'
})
export class LateComePolicyComponent {
                ...
                try {
                  ...
                  if (Array.isArray(permissionsData) && permissionsData.length > 0) {
                    ...
                    if (firstItem.is_superuser) {
                      ...
                      this.hasAddPermission = this.checkGroupPermission('add_latecomingpolicy', groupPermissions);
                      console.log('Has add permission:', this.hasAddPermission);
  
                      this.hasEditPermission = this.checkGroupPermission('change_latecomingpolicy', groupPermissions);
                      console.log('Has edit permission:', this.hasEditPermission);
  
                      this.hasDeletePermission = this.checkGroupPermission('delete_latecomingpolicy', groupPermissions);
                      console.log('Has delete permission:', this.hasDeletePermission);
  
  
                      this.hasViewPermission = this.checkGroupPermission('view_latecomingpolicy', groupPermissions);
  ...
  this.CountryService.getAttendancePolicy().subscribe({
    ...
    late_occurrence_limit: this.late_occurrence_limit,
    ...
    employee: selectedEmployeeIds,
      ...
      this.CountryService.registerlatecomingpolicy(companyData).subscribe(
        (response) => {
...
updateLateComingPolicy() {
 ...
 const payload = {
  ...
  late_occurrence_limit: Number(this.editAsset.late_occurrence_limit),
  ...
  this.CountryService
      ...
      .subscribe({
        ...
        next: () => {
          ...
          alert("Late Coming Policy Updated");
  ...
  deleteSelectedLateComeingPolicy() {
    const selectedEmployeeIds = this.LoanTypes
      .filter(employee => employee.selected)
      .map(employee => employee.id);

    if (selectedEmployeeIds.length === 0) {
      alert('No Late Coming Policy selected for deletion.');
      return;
    }

    if (confirm('Are you sure you want to delete the selected Late Coming Policy?')) {
      ...
      selectedEmployeeIds.forEach(categoryId => {
        this.CountryService.deleteLateComingPolicy(categoryId).subscribe(
          () => {
            console.log('Late Coming Policy deleted successfully:', categoryId);
            // Remove the deleted employee from the local list
            this.LoanTypes = this.LoanTypes.filter(employee => employee.id !== categoryId);
            completed++;
            if (completed === total) {
              alert('Late Coming Policy deleted successfully');
              window.location.reload();
            }

          },
          (error) => {
            console.error('Error deleting Late Coming Policy:', error);
            alert('Error deleting Late Coming Policy: ' + error.statusText);
          }
        );
      });
    }
