...
this.ApprovalLEvel.forEach(employee => employee.selected = this.allSelecteds);
...
openEditModal(asset: any): void {
  ...
  this.loadUsers(() => {
    this.mapApproverNameToId();   // 🔥 IMPORTANT
  });
...
deleteSelectedAssetType() { 
  const selectedEmployeeIds = this.ApprovalLEvel
    .filter(employee => employee.selected)
    .map(employee => employee.id);

  if (selectedEmployeeIds.length === 0) {
    alert('No Approval Levelselected for deletion.');
    return;
  }

  if (confirm('Are you sure you want to delete the selected  Approval Level?')) {
    selectedEmployeeIds.forEach(categoryId => {
      this.employeeService.deleteGenreqApprovalLevel(categoryId).subscribe(
        () => {
          console.log(' type deleted successfully:', categoryId);
          // Remove the deleted employee from the local list
          this.ApprovalLEvel = this.ApprovalLEvel.filter(employee => employee.id !== categoryId);
