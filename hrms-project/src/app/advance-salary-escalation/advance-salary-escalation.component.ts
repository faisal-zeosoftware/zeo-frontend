...
@Component({
  ...
  templateUrl: './advance-salary-escalation.component.html',
  styleUrl: './advance-salary-escalation.component.css'
})
export class AdvanceSalaryEscalationComponent {
  ...
  resetSelectedGeneralReq() {
    ...
    if (confirm('Are you sure you want to reset the selected General Request(s)?')) {
      ...
      selectedIds.forEach(id => {
        this.employeeService.resetAdvSalEsc(id).subscribe(
          () => {
    ...
    console.log("Mapped employee_id:", this.editAsset.escalate_to);
            ...
            registerGeneralreq() {
              ...
              this.DepartmentServiceService
                  .updateEscalationLevelAdv(selectedSchema, levelId, payload)
                  .subscribe(
                    (res) => {
  ...
  onRequestTypeChange(event: any) {
    ...
    this.DepartmentServiceService.getEscalationLevelsAdvSalary(selectedSchema, selectedTypeId)
      .subscribe(
        (result: any) => {
