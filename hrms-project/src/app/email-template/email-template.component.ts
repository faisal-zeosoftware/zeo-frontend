...
getBranchNames(branchIds: number[]): string {
  ...
  loadRequestType(): void {
    ...
    loadEmailPlaceholders(callback?: Function): void {
      ...
      // Check if selectedSchema is available
      if (selectedSchema) {
        this.employeeService.getEmailPlaceholderNew(selectedSchema,savedIds).subscribe(
          (result: any) => {
 ...
 let errorMessage = 'Enter all required fields!';
    ...
    //   }
      ...
      fetchEmployees(schema: string, branchIds: number[]): void {
        this.isLoading = true;
        this.DepartmentServiceService.getEmailTemplatesNew(schema, branchIds).subscribe({
          next: (data: any) => {
