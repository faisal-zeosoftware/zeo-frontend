...
// 1. Update your Filter Fields to match the JSON data keys
filterFields: string[] = ['emp_branch_id', 'status', 'request_type', 'branch'];
...
async initialLoad() {
  this.isLoading = true;
  try {
    const fieldRes = await this.leaveService.getAvailableFieldsGeneralRequest().toPromise();
...
extractUniqueStatuses() {
  this.filterFields.forEach(field => {
    // Get unique values for this specific field from originalData
    const uniqueValues = [...new Set(this.originalData.map(item => item[field]))]
      .filter(val => val !== null && val !== undefined && val !== '');
    
    // Initialize the object for this field if it doesn't exist
    if (!this.activeFilters[field]) {
      this.activeFilters[field] = {};
    }

    // Set each unique value to false (unchecked) by default
...
applyFilters() {
  ...
  // Multi-select Dynamic Filtering
  this.filterFields.forEach(field => {
    const selectedValues = Object.keys(this.activeFilters[field] || {})
      .filter(key => this.activeFilters[field][key]);

    if (selectedValues.length > 0) {
      temp = temp.filter(item => selectedValues.includes(String(item[field])));
    }
  });

  // Date Range Filtering (Using created_at_date from your JSON)
  if (this.startDate || this.endDate) {
    temp = temp.filter(item => {
      if (!item.created_at_date) return false;
      const itemDate = new Date(item.created_at_date);
...
saveCustomFile(): void {
  ...
  // 2. Prepare the URL
  const url = `${this.apiUrl}/employee/api/report-general-request/generate_general_report/?schema=${selectedSchema}`;
...
downloadExcel() {
  ...
  const fileName = this.customFileName || 'General_Request_Report';
...
get groupedData(): any[] {
  // IF NO GROUPING: Return the flat list. 
  // The template will see there is no "groupName" and render the rows directly.
  ...
  const groupRecursive = (data: any[], groupByKeys: string[], level: number): any[] => {
    ...
    return Object.keys(groups).map(name => ({
      groupName: name,
      level: level,
      // If there are more keys, recurse. If not, the children are the actual data objects.
...
deleteReport(report: any, event: Event) {
  ...
  if (report.id === 1 || report.file_name === 'generalrequest_std_report') {
  ...
  if (confirm(`Are you sure you want to delete "${report.file_name}"?`)) {
    this.isLoading = true;
    this.leaveService.deleteGenReqReport(report.id).subscribe({
      next: (res) => {
        this.isLoading = false;
