...
@Component({
  selector: 'app-general-request-report',
  templateUrl: './general-request-report.component.html',
...
public datePipe: DatePipe;
  ...
  // dateRange: { start: Date | null, end: Date | null } = { start: null, end: null };
  dateRange: { start: Date | null, end: Date | null } = { start: null, end: null };
  dateRanges: { start: Date | null; end: Date | null } = { start: null, end: null };
                ...
                if (Array.isArray(permissionsData) && permissionsData.length > 0) {
                  ...
                  if (firstItem.is_superuser) {
                    ...
                    console.log('Group Permissions:', groupPermissions);
                      ...
                      this.hasViewPermission = this.checkGroupPermission('view_generalrequestreport', groupPermissions);
                 console.log('Has view permission:', this.hasViewPermission);
            
                   this.hasAddPermission = this.checkGroupPermission('add_generalrequestreport', groupPermissions);
                  console.log('Has add permission:', this.hasAddPermission);
            
                 this.hasDeletePermission = this.checkGroupPermission('delete_generalrequestreport', groupPermissions);
              console.log('Has delete permission:', this.hasDeletePermission);
            
                  this.hasEditPermission = this.checkGroupPermission('change_generalrequestreport', groupPermissions);
                 console.log('Has edit permission:', this.hasEditPermission);
                 
                 this.hasExportPermission = this.checkGroupPermission('export_general_request_report', groupPermissions);
  ...
  onStartDateChanges(event: any) {
    this.dateRanges.start = event.value;
  }

  onEndDateChanges(event: any) {
    this.dateRanges.end = event.value;
  }
...
StandardDownload(): void {
  ...
  try {
    // Define display names for headers (modify as needed)
    ...
    const filteredData = this.reportData.data.filter((item: any) => {
      const activeDate = new Date(item.created_at_date);
      return (!this.startDate || activeDate >= this.startDate) &&
             (!this.endDate || activeDate <= this.endDate);
    });

    // Log filtered data for debugging (optional)
    console.log('Filtered Data:', filteredData);

    // Map data to use display names and exclude missing properties
    ...
    // Log mapped data for debugging (optional)
    console.log('Mapped Data:', mappedData);

    // Create worksheet rows with data using mapped names
    mappedData.forEach(row => {
      const values = headers.map(header => row[header] || '');
      worksheetData.push(values);
    });

    // Create the worksheet using SheetJS library
    const worksheet: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet(worksheetData);

    // Add a merged cell for the title
    if (!worksheet['!merges']) worksheet['!merges'] = [];
    worksheet['!merges'].push({ s: { r: 0, c: 0 }, e: { r: 0, c: headers.length - 1 } });

    // Set formatting for the title cell
    worksheet['A1'].s = {
    ...
    // Create the workbook and convert it to an array buffer
    const workbook: XLSX.WorkBook = { Sheets: { 'data': worksheet }, SheetNames: ['data'] };
    const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });

    // Save the Excel file (replace with your implementation)
...
saveAsExcelFile(buffer: any, fileName: string): void {
 ...
 private saveExcelFile(buffer: any, fileName: string): void {
  ...
  //   { value: 'document_type', displayName: 'Document Type', selected: false },
  ...
  loadAvailableFields(): void {
    this.reportgenerateService.getGeneralRequestFields().subscribe(
    ...
    this.http.post<any>(url, formData).subscribe(
      response => {
        ...
        if (response && response.unique_values) {
          console.log('Unique Values:', response.unique_values); // Log unique values

          const filteredUniqueValues = this.filterValuesByDateRange(response.unique_values, startDateFormatted!, endDateFormatted!);
          console.log('Filtered Unique Values:', filteredUniqueValues); // Log filtered values

          this.jsonDatas = Object.entries(filteredUniqueValues).map(([key, value]) => ({
            key,
            values: Array.isArray(value) ? value.map(v => this.formatValue(v)) : [this.formatValue(value)]
          }));

          console.log('Formatted JSON Data:', this.jsonDatas); // Log final jsonDatas
  ...
  filterValuesByDateRange(uniqueValues: { [key: string]: any[] }, startDate: string, endDate: string): { [key: string]: any[] } {
    ...
    Object.keys(uniqueValues).forEach(key => {
      ...
      if (Array.isArray(values)) {
        filteredValues[key] = values.filter(value => {
          const date = new Date(value);
          return date >= start && date <= end;
        });
      } else {
        console.warn(`Expected array but received ${typeof values} for key: ${key}`);
        // Convert single object values to array for consistency
        filteredValues[key] = values ? [values] : []; // Ensure there's at least an empty array
      }
    });
  ...
  formatValue(value: any): string {
    ...
    return String(value); // Convert to string if it's not an object
  }
  ...
  //   const url = `http://${selectedSchema}.localhost:8000/employee/api/report-general-request/generate_filter_table/`;
  //   const formData = new FormData();
  //   formData.append('report_id', this.selectedReportId.toString());
  ...
  //         this.openReportDialog(this.selectedReportId,this.uniqueValues); // Ensure it's a number
