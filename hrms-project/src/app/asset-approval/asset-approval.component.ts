...
export class AssetApprovalComponent {
  ...
  fetchEmployees(schema: string, branchIds: number[]): void {
    this.isLoading = true;
    this.EmployeeService.getApprovalslistAssetNew(schema, branchIds).subscribe({
      next: (data: any) => {
  ...
  sendDelegationResponseInline(apr: any): void {
    ...
    const apiUrl = `${this.apiUrl}/organisation/api/asset-request-approvals/${apr.id}/send_response/?schema=${selectedSchema}`;
    const payload = {
  ...
  createDelegation(): void {
    ...
    const apiUrl = `${this.apiUrl}/organisation/api/asset-request-approvals/${this.selectedApproval.id}/delegate/?schema=${selectedSchema}`;
    const payload = {
  ...
  applyFilters(): void {
    ...
    this.filteredApprovals = this.allApprovals.filter(apr => {
      const matchesSearch =
        !search ||
        (apr.asset_request ?? '').toLowerCase().includes(search) ||
  ...
  getEmptyMessage(): string {
    ...
    if (this.searchText && this.selectedStatus) {
      return `No ${this.selectedStatus.toLowerCase()} asset requests matching "${this.searchText}".`;
    }
    if (this.searchText) {
      return `No asset requests matching "${this.searchText}".`;
    }
    if (this.selectedStatus) {
      return `No ${this.selectedStatus.toLowerCase()} asset requests found.`;
    }
    return 'No asset requests found.';
  }
