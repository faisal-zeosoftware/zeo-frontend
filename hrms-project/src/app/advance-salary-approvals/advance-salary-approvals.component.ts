...
@Component({
  selector: 'app-advance-salary-approvals',
  templateUrl: './advance-salary-approvals.component.html',
  styleUrl: './advance-salary-approvals.component.css'
})
export class AdvanceSalaryApprovalsComponent {
...
//       this.EmployeeService.getApprovalslistadvSalary(selectedSchema, this.userId).subscribe(
//           (result: any) => {
...
LoadEmployee(selectedSchema: string) {
  this.leaveService.getEmployee(selectedSchema).subscribe(
    ...
    if (selectedSchema) {
      this.leaveService.getAllAdvSalRequest(selectedSchema).subscribe(
        (result: any) => {
...
selectedDelegationId: number | null = null;
  ...
  sendDelegationResponseInline(apr: any): void {
    ...
    const apiUrl =
      `${this.apiUrl}/payroll/api/approval-salaryrequest/${apr.id}/send_response/?schema=${selectedSchema}`;
  ...
  canShowResponse(apr: any): boolean {
   ...
   createDelegation(): void {
    ...
    const apiUrl =
      `${this.apiUrl}/payroll/api/approval-salaryrequest/${this.selectedApproval.id}/delegate/?schema=${selectedSchema}`;
  ...
  openDelegationModalFromApproval(approval: any) {
    ...
    const advsalaryRequest = this.Assetreq.find(
      (req: any) => req.document_number === approval.request
    );
    ...
    this.delegationForm = {
    request: advsalaryRequest ? advsalaryRequest.id : null,
