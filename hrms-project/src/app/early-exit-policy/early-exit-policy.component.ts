...
@Component({
  selector: 'app-early-exit-policy',
  templateUrl: './early-exit-policy.component.html',
  styleUrl: './early-exit-policy.component.css'
})
export class EarlyExitPolicyComponent {
      ...
      reset_monthly: boolean = false;
  
      leave_days_to_deduct: any = '';
      occurrence_limit: any = '';
      penalty_type: any = '';
                  ...
                  try {
                    ...
                    if (Array.isArray(permissionsData) && permissionsData.length > 0) {
                      ...
                      if (firstItem.is_superuser) {
                        ...
                        this.hasAddPermission = this.checkGroupPermission('add_earlyexitpolicy', groupPermissions);
                        console.log('Has add permission:', this.hasAddPermission);
    
                        this.hasEditPermission = this.checkGroupPermission('change_earlyexitpolicy', groupPermissions);
                        console.log('Has edit permission:', this.hasEditPermission);
    
                        this.hasDeletePermission = this.checkGroupPermission('delete_earlyexitpolicy', groupPermissions);
                        console.log('Has delete permission:', this.hasDeletePermission);
    
    
                        this.hasViewPermission = this.checkGroupPermission('view_earlyexitpolicy', groupPermissions);
    ...
    this.CountryService.getAttendancePolicy().subscribe({
      next: (res: any) => {
        console.log('Attendance Policies:', res);
        this.attendancePolicies = res;
      },
      error: (err) => {
        console.error(err);
      }
      ...
      const companyData = {
      leave_days_to_deduct: this.leave_days_to_deduct,
      occurrence_limit: this.occurrence_limit,
      ...
      employee: selectedEmployeeIds,
        ...
        this.CountryService.registerearlyexitpolicy(companyData).subscribe(
          (response) => {
            console.log('Registration successful', response);
    
            alert('early exit policy Added ');
  ...
  updateEarlyExitPolicy() {
   ...
   const payload = {
    ...
    attendance_policy: this.editAsset.attendance_policy,
    occurrence_limit: Number(this.editAsset.occurrence_limit),
    leave_days_to_deduct: Number(this.editAsset.leave_days_to_deduct),
    penalty_type: this.editAsset.penalty_type,
  
    enabled: this.editAsset.enabled,
    reset_monthly: this.editAsset.reset_monthly,
    ...
    this.CountryService
        ...
        .subscribe({
          ...
          next: () => {
            ...
            alert("Early Exit Policy Updated");
    ...
    deleteSelectedEarlyexitPolicy() {
      const selectedEmployeeIds = this.LoanTypes
        .filter(employee => employee.selected)
        .map(employee => employee.id);
  
      if (selectedEmployeeIds.length === 0) {
        alert('No Early Exit Policy selected for deletion.');
        return;
      }
  
      if (confirm('Are you sure you want to delete the selected Early Exit Policy?')) {
        ...
        selectedEmployeeIds.forEach(categoryId => {
          this.CountryService.deleteEarlyexitPolicy(categoryId).subscribe(
            () => {
              console.log('Early Exit Policy deleted successfully:', categoryId);
              // Remove the deleted employee from the local list
              this.LoanTypes = this.LoanTypes.filter(employee => employee.id !== categoryId);
              completed++;
              if (completed === total) {
                alert('Early Exit Policy deleted successfully');
                window.location.reload();
              }
  
            },
            (error) => {
              console.error('Error deleting Early Exit Policy:', error);
              alert('Error deleting Early Exit Policy: ' + error.statusText);
            }
          );
        });
      }
    ...
    filterBranches(): any[] {
      ...
      return this.Categories.filter((selectedCategories: any) =>
        selectedCategories.ctgry_title.toLowerCase().includes(search)
      );
    }
      ...
      return this.Designations.filter((selectedDesignations: any) =>
        selectedDesignations.desgntn_job_title.toLowerCase().includes(search)
      );
    }
