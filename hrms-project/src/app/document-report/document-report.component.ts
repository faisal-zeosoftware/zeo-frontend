...
filterFields: string[] = ['emp_branch_id', 'emp_dept_id', 'document_type', 'is_active'];
dateField: string = 'emp_doc_issued_date'; // The key from your JSON to filter by date
...
async initialLoad() {
  this.isLoading = true;
  try {
    const fieldRes = await this.leaveService.getAvailableFieldsDocument().toPromise();
...
fetchSavedReportsList() {
  this.leaveService.getDocumentReport().subscribe(res => {
...
//       const defaultReport = res.find(report => report.file_name === 'doc_std_report');
...
fetchStandardReport(schema: string, branchIds: number[]): void {
  this.isLoading = true;
  this.leaveService.getDocumentReportNew(schema, branchIds).subscribe(res => {
    if (res && res.length > 0) {
      // 1. Find the report where file_name is 'std_report'
...
applyFilters() {
  ...
  // 2. Dynamic Checkbox Filtering (Branch, Dept, etc.)
  this.filterFields.forEach(field => {
    ...
    if (selectedValues.length > 0) {
      temp = temp.filter(item => {
        const itemValue = String(item[field] === null ? 'Unassigned' : item[field]);
        return selectedValues.includes(itemValue);
      });
    }
  });

  // 3. Date Range Filtering (using emp_doc_issued_date)
...
downloadExcel() {
  ...
  const fileName = this.customFileName || 'Document_Report';
...
get groupedData(): any[] {
  // If no grouping is selected, wrap all data in one virtual group for the template
  ...
  const groupRecursive = (data: any[], groupByKeys: string[], level: number): any[] => {
    ...
    return Object.keys(groups).map(name => ({
      groupName: name,
      level: level,
      // If there are more levels to group by, keep recursing
...
deleteReport(report: any, event: Event) {
  ...
  if (report.id === 1 || report.file_name === 'doc_std_report') {
