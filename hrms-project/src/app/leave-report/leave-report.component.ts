...
async initialLoad() {
  this.isLoading = true;
  try {
    const fieldRes = await this.leaveService.getAvailableFieldsLeaveReport().toPromise();
...
fetchSavedReportsList() {
  this.leaveService.getLeaveReport().subscribe(res => {
...
//       const defaultReport = res.find(report => report.file_name === 'leave_std_report');
...
fetchStandardReport(schema: string, branchIds: number[]): void {
  this.isLoading = true;
  this.leaveService.getLeaveReportNew(schema, branchIds).subscribe(res => {
    if (res && res.length > 0) {
      // 1. Find the report where file_name is 'std_report'
...
applyFilters() {
  ...
  // 2. Dynamic Checkbox Filtering (Leave Type, Status, Branch)
  this.filterFields.forEach(field => {
    ...
    if (selectedValues.length > 0) {
      temp = temp.filter(item => {
        // Handle null values by treating them as 'Unassigned' to match extractUniqueStatuses
        const itemValue = (item[field] === null || item[field] === undefined || item[field] === '') 
                          ? 'Unassigned' 
                          : String(item[field]);
  ...
  // 3. Date Range Filtering (Comparing against start_date)
...
downloadExcel() {
  ...
  const fileName = this.customFileName || 'Leave_Report';
...
deleteReport(report: any, event: Event) {
  ...
  if (report.id === 1 || report.file_name === 'leave_std_report') {
