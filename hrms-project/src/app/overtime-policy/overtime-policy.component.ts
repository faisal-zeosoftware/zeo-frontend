...
@Component({
  ...
  styleUrl: './overtime-policy.component.css'
})
...
this.department.forEach((id: number) =>
  formData.append('department', id.toString())
);

this.designation.forEach((id: number) =>
  formData.append('designation', id.toString())
);

this.category.forEach((id: number) =>
  formData.append('category', id.toString())
);

  this.employeeService.registerOverTimePolicy(formData).subscribe(
    (response) => {
      console.log('Registration successful', response);
      alert('OverTime Policy has been added');
  ...
  deleteSelectedPayrollAprlvl() {
    ...
    if (confirm('Are you sure you want to delete the selected OverTime Policy ?')) {

    let total = selectedEmployeeIds.length;
    let completed = 0;

      selectedEmployeeIds.forEach(categoryId => {
        this.employeeService.deleteOvertimepolicy(categoryId).subscribe(
          () => {
  ...
  updateOverTimePolicy(): void {
    ...
    this.employeeService.updatelovertimepolicy(this.editAsset.id, this.editAsset).subscribe(
      (response) => {
