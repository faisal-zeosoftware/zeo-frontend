            ...
            try {
              ...
              if (Array.isArray(permissionsData) && permissionsData.length > 0) {
                ...
                if (firstItem.is_superuser) {
                 ...
                 this.hasAddPermission = this.checkGroupPermission('add_leaveapprovalreport', groupPermissions);
                 console.log('Has add permission:', this.hasAddPermission);
                  
                 this.hasEditPermission = this.checkGroupPermission('change_leaveapprovalreport', groupPermissions);
                 console.log('Has edit permission:', this.hasEditPermission);
    
                 this.hasDeletePermission = this.checkGroupPermission('delete_leaveapprovalreport', groupPermissions);
                 console.log('Has delete permission:', this.hasDeletePermission);
    
  
                 this.hasViewPermission = this.checkGroupPermission('view_leaveapprovalreport', groupPermissions);
...
fetchStandardReport(schema: string, branchIds: number[]): void {
  this.isLoading = true;
  this.leaveService.getLeaveApprovalReportNew(schema, branchIds).subscribe(res => {
    if (res && res.length > 0) {
      // 1. Find the report where file_name is 'std_report'
