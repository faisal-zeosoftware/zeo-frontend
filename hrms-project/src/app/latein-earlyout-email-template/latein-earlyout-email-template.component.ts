...
@Component({
  selector: 'app-latein-earlyout-email-template',
  templateUrl: './latein-earlyout-email-template.component.html',
  styleUrl: './latein-earlyout-email-template.component.css'
})
export class LateinEarlyoutEmailTemplateComponent {
  ...
  loadEmailPlaceholders(): void {
    ...
    // Check if selectedSchema is available
    if (selectedSchema) {
      this.companyservice.getEmailPlaceholderLateInEarlyOut(selectedSchema).subscribe(
        (result: any) => {
  ...
  onclickEmailTemplate(): void {
    ...
    const companyData = {
      ...
      Designation: this.designation,// Use the captured Summernote content here
    ...
    this.companyservice.registerEmailTemplateLateInEarlyOut(companyData).subscribe(
      (response) => {
...
toggleAllDesignations(): void {
  ...
  //   // Check if selectedSchema is available
  //   if (selectedSchema) {
  //     this.DepartmentServiceService.getEmailTemplatesResignation(selectedSchema).subscribe(
  //       (result: any) => {
  ...
  fetchEmployees(schema: string, branchIds: number[]): void {
    this.isLoading = true;
    this.companyservice.getEmailTemplatesLateInEarlyOutNew(schema, branchIds).subscribe({
      next: (data: any) => {
  ...
  openEditPopuss(selectedTemplate: any): void {
    this.dialog.open(LateinEarlyoutEmailTemplateEditComponent, {
      width: '80%',
      height: '700px',
...
toggleAllSelectionBrach(): void {
  if (this.selectBrach) {
    this.selectBrach.options.forEach((item: MatOption) =>
  ...
  deleteSelectedLateInEarlyOut() {
    ...
    if (confirm('Are you sure you want to delete the selected Email Template?')) {
      ...
      selectedEmployeeIds.forEach(categoryId => {
        this.companyservice.deleteEmailTemplateLateInEarlyOut(categoryId).subscribe(
          () => {
