...
deleteLeaveApplicable(id: number): Observable<any> {
  ...
  getCompanLeave(selectedSchema: string, branchIds: number[]): Observable<any> {
    // Converts [1,3,4] into the string "[1,3,4]" for the URL
    const branchParam = branchIds.length > 0 ? `[${branchIds.join(',')}]` : '';
    
    let url = `${this.apiUrl}/calendars/api/compensatory-leave-allocation/?schema=${selectedSchema}`;
    if (branchParam) {
      url += `&branch_id=${branchParam}`;
    }
...
CreateLeaveapprovalLevel(payload: any): Observable<any> {
  const selectedSchema = localStorage.getItem('selectedSchema');
...
CreateDocRequestapprovalLevel(data: any): Observable<any> {
  ...
  return this.http.post(apiUrl, data).pipe(
    catchError((error) => {
      ...
      getemployeesDocumentrequestApprovalLevel(selectedSchema: string, branchIds: number[]): Observable<any> {
    // Converts [1,3,4] into the string "[1,3,4]" for the URL
    const branchParam = branchIds.length > 0 ? `[${branchIds.join(',')}]` : '';
    
    let url = `${this.apiUrl}/employee/api/Doc-request-approval-levels/?schema=${selectedSchema}`;
    if (branchParam) {
      url += `&branch_id=${branchParam}`;
    }
