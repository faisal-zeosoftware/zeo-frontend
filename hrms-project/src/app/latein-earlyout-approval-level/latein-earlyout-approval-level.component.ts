...
@Component({
  selector: 'app-latein-earlyout-approval-level',
  templateUrl: './latein-earlyout-approval-level.component.html',
  styleUrl: './latein-earlyout-approval-level.component.css'
})
export class LateinEarlyoutApprovalLevelComponent {
  ...
  private employeeService: EmployeeService,
                ...
                try {
                  ...
                  if (Array.isArray(permissionsData) && permissionsData.length > 0) {
                    ...
                    if (firstItem.is_superuser) {
                      ...
                      this.hasAddPermission = this.checkGroupPermission('add_lateinearlyoutapprovallevel', groupPermissions);
                      console.log('Has add permission:', this.hasAddPermission);
                      
                      this.hasEditPermission = this.checkGroupPermission('change_lateinearlyoutapprovallevel', groupPermissions);
                      console.log('Has edit permission:', this.hasEditPermission);
        
                     this.hasDeletePermission = this.checkGroupPermission('delete_lateinearlyoutapprovallevel', groupPermissions);
                     console.log('Has delete permission:', this.hasDeletePermission);
        
      
                      this.hasViewPermission = this.checkGroupPermission('view_lateinearlyoutapprovallevel', groupPermissions);
    ...
    //   }
      ...
      fetchEmployees(schema: string, branchIds: number[]): void {
        this.isLoading = true;
        this.employeeService.getemployeesLateInEarlyOutApprovalLevel(schema, branchIds).subscribe({
          next: (data: any) => {
  ...
  deleteSelectedLateInEarlyOutAprlvl() { 
    const selectedEmployeeIds = this.approvalLevels
      .filter(employee => employee.selected)
      .map(employee => employee.id);
  
    if (selectedEmployeeIds.length === 0) {
      alert('No Late In Early Out Approval Level selected for deletion.');
      return;
    }
  
    if (confirm('Are you sure you want to delete the selected Late In Early Out ?')) {
      ...
      selectedEmployeeIds.forEach(categoryId => {
        this.employeeService.deleteLateInEarlyOutApprovaLevel(categoryId).subscribe(
          () => {
  ...
  if (completed === total) {
          ...
          (error) => {
            console.error('Error deleting Late In Early Out:', error);
           alert('Error deleting Late In Early Out: ' + error.statusText);
