...
@Component({
  ...
  templateUrl: './airticket-approvals.component.html',
  styleUrl: './airticket-approvals.component.css'
})
...
export class AirticketApprovalsComponent {
 ...
 this.dataSubscription = combineLatest([
  ...
  loadUsers(): void {
    ...
    if (selectedSchema) {
      this.leaveService.getAllairRequest(selectedSchema).subscribe(
        (result: any) => {
          this.Airreq = result;
...
selectedDelegationId: number | null = null;
  ...
  sendDelegationResponseInline(apr: any): void {
    ...
    const apiUrl =
      `${this.apiUrl}/payroll/api/airticket-approval/${apr.id}/send_response/?schema=${selectedSchema}`;
  ...
  canShowResponse(apr: any): boolean {
   ...
   createDelegation(): void {
    ...
    const apiUrl =
      `${this.apiUrl}/payroll/api/airticket-approval/${this.selectedApproval.id}/delegate/?schema=${selectedSchema}`;
  ...
  openDelegationModalFromApproval(approval: any) {
    ...
    const airRequest = this.Airreq.find(
  ...
  this.delegationForm = {
    request: airRequest ? airRequest.id : null,
