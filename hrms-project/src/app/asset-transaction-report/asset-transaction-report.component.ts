...
filterFields: string[] = ['emp_branch_id', 'asset', 'return_condition', 'emp_dept_id'];
...
async initialLoad() {
  this.isLoading = true;
  try {
    const fieldRes = await this.leaveService.getAvailableFieldsAssetTransaction().toPromise();
...
applyFilters() {
  ...
  // 2. Dynamic Field Filtering (Status, Branch, etc.)
  this.filterFields.forEach(field => {
  ...
  if (this.startDate || this.endDate) {
    temp = temp.filter(item => {
      const itemDate = item.assigned_date ? new Date(item.assigned_date) : null;
...
saveCustomFile(): void {
  ...
  // 2. Prepare the URL
  const url = `${this.apiUrl}/organisation/api/asset-transaction-report/generate_general_report/?schema=${selectedSchema}`;
