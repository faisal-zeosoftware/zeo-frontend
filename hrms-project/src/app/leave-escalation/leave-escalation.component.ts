...
@Component({
  ...
  styleUrl: './leave-escalation.component.css'
})
      ...
      if (schema) {
        ...
        resetSelectedGeneralReq() {
          ...
          if (selectedIds.length === 0) {
            alert('No Leave Esc selected for reset.');
            return;
          }
          ...
          if (confirm('Are you sure you want to reset the selected General Request(s)?')) {
            ...
            selectedIds.forEach(id => {
              this.employeeService.resetLeaveEsc(id).subscribe(
                () => {
                  console.log('Leave Esc reset successfully:', id);
                  completed++;
        
                  if (completed === total) {
                    alert('Leave Esc reset successfully');
                    window.location.reload();
                  }
                },
                (error) => {
          ...
          console.log("Mapped employee_id:", this.editAsset.escalate_to);
                  ...
                  registerGeneralreq() {
                    ...
                    this.DepartmentServiceService
                        .updateEscalationLevelLeave(selectedSchema, levelId, payload)
                        .subscribe(
                          (res) => {
        ...
        onRequestTypeChange(event: any) {
          ...
          this.DepartmentServiceService.getEscalationLevelsLeave(selectedSchema, selectedTypeId)
            .subscribe(
              (result: any) => {
