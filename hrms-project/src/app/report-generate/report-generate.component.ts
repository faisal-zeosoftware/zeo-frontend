...
private apiUrl = `${environment.apiBaseUrl}`; // Use the correct `apiBaseUrl` for live and local
  ...
  public baseUrls = 'http://127.0.0.1:8000/emp-report'; // Define the base URL property
  ...
  fetchStandardReport(): void {
    this.reportService.getStandardReport().subscribe(
      (response: any) => {
...
saveAsExcelFile(buffer: any, fileName: string): void {
 ...
 private saveExcelFile(buffer: any, fileName: string): void {
  ...
  loadAvailableFields(): void {
    this.reportgenerateService.getEmployeeFields().subscribe(
